...
bool PhysBody::Contains(int x, int y) const
{
	...
	float a = METERS_TO_PIXELS(x);
	float b = METERS_TO_PIXELS(y);
	b2Vec2 point(a, b);
...
int PhysBody::RayCast(int x1, int y1, int x2, int y2, float& normal_x, float& normal_y) const
{
	...
	float a = METERS_TO_PIXELS(x1);
	float b = METERS_TO_PIXELS(y1);
	float c = METERS_TO_PIXELS(x2);
	float d = METERS_TO_PIXELS(y2);
	b2Fixture *fixture = body->GetFixtureList();
	b2Vec2 point1(a, b);
	b2Vec2 point2(c, d);
