...
bool ModulePhysics::CreateRectangles()
{
		b2BodyDef body;
		float a = PIXEL_TO_METERS(30);
		float h = PIXEL_TO_METERS(20);
		body.type = b2_dynamicBody;
		body.position.Set(PIXEL_TO_METERS(App->input->GetMouseX()), PIXEL_TO_METERS(App->input->GetMouseY()));
		b2Body* b = world->CreateBody(&body);
...
bool ModulePhysics::CreateChains()
{
		...
		for (int i = 0; i < 12; i++)
		{
			...
			vec[i].y = PIXEL_TO_METERS(points[j + 1]);
			j += 2;
		}
	...
	int Rick[56] = {
		...
		for (int i = 0; i < 28; i++)
		{
			vec[i].x = PIXEL_TO_METERS(Rick[j]);
			vec[i].y = PIXEL_TO_METERS(Rick[j + 1]);
			j += 2;
		}
